...
# 데이터를 로드하고 월별 수익율 계산하여 새로운 데이터프레임에 추가하는 함수
def data_load(
        _path = "./data",
        _ext = 'csv',
        _start = '2010-01-01',
    ...
    for file in files:
        ...
        # 함수 호출
        price_df, ym_list = create_lm_rtn(read_df, head, _start, _end, _col)
...
def multi_return(_df, _codes):
    ...
    # index를 기준으로 반복문 생성 => 날짜별 매두, 매도 확인
    for idx in df.index:
        # 종목별로 매수, 매도를 확인
        for code in _codes:
            ...
            if (df.shift(2).loc[idx, f"p_{code}"] == "") & (df.shift().loc[idx, f"p_{code}"] == f"ready_{code}") & \
                (df.loc[idx, f"p_{code}"] == f"buy_{code}"):
